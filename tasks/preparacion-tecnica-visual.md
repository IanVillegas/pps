...
| D-03 | Dos composiciones del paso 1 y capas duplicadas de shell       | **Resuelto 2026-09-22**: se implemento la composicion inferior (DEC-007), confirmada campo por campo con `get_design_context` real. Correccion sobre lo que decia el inventario: de los 11 campos, solo "Estado civil" y "Agencia" son Dropdown-NEW (select); "Ultimo grado academico obtenido" y "Oficina para la que labora" son texto libre. **Ajuste 2026-09-22 (mismo dia)**: el usuario confirmo que a "Direccion actual (exacta)" le faltaba el asterisco en el propio Figma (error de diseno, no de lectura) -- los 11 campos son obligatorios, ninguno opcional. |
...
| D-09 | Catalogos, datos precargados, editabilidad y origen de saldos  | Acordar con backend por paso; no se deduce de valores de ejemplo. **Confirmado 2026-09-22 por el usuario**: los catalogos reales (estado civil, grado academico, agencia, oficina, parentesco, tipo de bien, etc.) todavia no estan definidos. Mientras no lleguen, cada Select se llena con opciones sinteticas identificables como mock en el propio codigo (mismo criterio que `AuthService`/`DeclarationService`), reemplazables sin tocar el resto del componente. No inventar nombres reales de agencias/oficinas de Grupo Mutual. |
...
| D-16 | Obligatoriedad en pasos de tabla (familia, inmuebles, muebles, ahorro, corrientes, creditos, tarjetas, personerias, sociedades) | **Resuelto 2026-09-22 por el usuario**: la tabla del paso puede quedar vacia — no tener ninguna fila no impide terminar la declaracion (ej. no tener bienes muebles). El asterisco de obligatoriedad aplica **dentro del modal de alta/edicion**, solo cuando se abre para agregar/editar una fila: ahi si son obligatorios todos los campos marcados. Consecuencia para el contrato de pasos (`stepDefinitions.tsx`, DEC-006B): el `validate` a nivel de paso de una tabla nunca debe exigir filas; la validacion de campos requeridos vive en el propio modal de la fila, no en el paso. Aplica a DEC-008B, 009B/009C, 010, 011, 012, 013, 014, 015, 016A y 016B. |
| D-17 | Limite de caracteres por campo de texto libre | **Resuelto 2026-09-22 por el usuario (valor simbolico)**: sin contrato de backend que defina el maximo real por campo, se usan valores simbolicos razonables en `src/utils/fieldLimits.ts` (`FIELD_MAX_LENGTH`), aplicados con el atributo nativo `maxLength` a cada campo de texto libre existente (Login: usuario/contraseña; DEC-007: los siete `InputText` y el `Textarea` de direccion). Un solo lugar para reemplazar por el valor real cuando exista contrato, sin tocar los componentes. Aplica tambien a cualquier campo de texto libre que se agregue en pasos futuros. |
| D-18 | Ancho de la tarjeta del wizard en pantallas anchas | **Revertido 2026-09-23, mismo dia**: la primera respuesta del usuario fue mantener `max-width: 1006px` literal de Figma. Al verlo en una Surface Pro 10 (1440px, viewport de referencia oficial) lo reporto "desplazado" (~100px de gris a la derecha) y pidio extenderlo. Se quito el `max-width`: la tarjeta ahora llena el ancho disponible, mismo patron que el panel de Inicio (ya no son dos criterios distintos). Verificado que 1366 sigue casi igual a Figma (1001px vs 1006px original) y que 768 (tablet) no desborda. Efecto secundario aceptado sin pedir cambio adicional: en monitores muy anchos (1920px) cada campo de una sola linea (ej. "Edad") queda ~674px, mas ancho de lo usual mas no roto visualmente. Aplica a la tarjeta de todos los pasos del wizard (`DeclarationWizard.module.scss`), no solo al paso 1. |
