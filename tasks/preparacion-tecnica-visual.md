...
| Campo de texto            | `Atoms/InputText`                             | **Resuelto 2026-09-14 con componente propio** (`src/components/Atoms/TextField`): a diferencia del boton, el label sin `htmlFor` y el error sin ARIA no son ajustables pasando props desde afuera, hay que cambiar el JSX interno; se reuso el diseno (colores/tipografia de GM) pero se escribio markup nuevo. `maxLength` nativo ya funciona pasandolo directo (`InputHTMLAttributes`).                                       |
