...
- **Corregido 2026-09-14** (misma logica que el episodio de DEC-002A): la version original tenia 24 variables, pero mas de la mitad (superficie, texto, bordes, alertas, tamanos de titulo) eran copias literales de valores que ya existen en `sad-aml-shared/styles/settings`, sin ninguna diferencia. Se recorto a las 9 que si son nuevas o representan una decision pendiente (rojo/verde 2026 vs heredado, el escalon de 12px que GM no tiene, y los radios que GM no centraliza). Un componente que necesite un color/tamano ya existente en GM debe usarlo directo con `@use`, no a traves de `--decpat-*`.
