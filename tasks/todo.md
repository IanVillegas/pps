...
- Ultima tarea implementada y validada: `DEC-005C Sidebar colapsable` (2026-09-23). Cierre en [DEC-005C](DEC-005C-sidebar-colapsable.md): tarea nueva pedida por el usuario (no estaba en este archivo), para liberar ancho horizontal a las tablas grandes de pasos futuros. Colapso automatico en tablet (mismo breakpoint 1100px de DEC-005A) + boton manual a cualquier ancho; colapsado queda una franja de solo iconos con Tooltip. De paso se corrigio un bug real de especificidad CSS (`&--collapsed` en SCSS genera un selector independiente, no compuesto con `.sideBar`) que impedia que el colapso automatico se aplicara.
- `DEC-007 Personales`: cerrada 2026-09-22, ver [DEC-007](DEC-007-personales.md) (paso 1 completo, D-03 resuelto con el diseno real, mas dos ajustes el mismo dia: `address` si es obligatorio, y limites de caracteres simbolicos en `fieldLimits.ts` D-17).
...
- Siguiente tarea sugerida: `DEC-008 Familia` (paso 2) o `DEC-002E/002F` (diferidas). `DEC-008B` ya tiene la regla de obligatoriedad resuelta (D-16): la tabla puede quedar vacia, el modal de alta valida sus propios campos. La autenticacion sigue simulada hasta disponer de contrato backend (D-08). `build` esta pendiente de confirmar en DEC-005C: no se corrio porque el servidor dev del usuario (puerto 3004) estaba activo.
...
| [x] DEC-005C Sidebar colapsable | Colapso automatico en tablet + boton manual; franja de solo iconos con nombre accesible y Tooltip; no oculta contenido de navegacion | 005A         | sad-aml-shared/components/Organisms/SideBar, src/components/Templates/AppShell           | S; V1,V3        |
...
| [x] DEC-002C2 Select   | Nombre accesible, teclado, seleccion controlada y error; corrige el bug de keys aleatorias (`Math.random()`) de `sad-aml-shared/Dropdown`, que no es ajustable desde afuera | 002C         | sad-aml-shared/components/Molecules/Dropdown, src/components/Molecules                                                                        | M; V1,V3        |
| [x] DEC-007 Personales | Una composicion D-03; once campos (solo estado civil y agencia son select, corregido contra el diseno real), obligatoriedad/editabilidad acordadas; ida/vuelta sin perdida | 006B,002C2   | src/components/Pages/Declaration/PersonalData, src/types/PersonalData.types.ts, src/services/CatalogService.ts, sad-aml-shared/components/Atoms/Textarea | M; V1,V3        |
...
- [ ] Primer paso conectado al wizard; errores impiden avanzar y estado se conserva. Contratos de catalogos revisados.
  - Primera mitad cumplida 2026-09-22 (DEC-007): paso 1 conectado, Continuar bloquea con campos vacios y anuncia el error, ida/vuelta sin perdida de datos. Los catalogos de Estado civil/Agencia siguen siendo mock (D-09 abierta); el checkpoint no se marca completo hasta que se revisen contratos reales.
