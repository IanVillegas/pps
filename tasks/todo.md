...
- Ultima tarea implementada y validada: `DEC-002C2 Select` (2026-09-22). Cierre en [DEC-002C2](DEC-002C2-select.md): se corrigio `sad-aml-shared/Dropdown` en el sitio (keys aleatorias con `Math.random()`, sin nombre accesible real, sin soporte de error/ref) en vez de crear un `SelectField` propio, por el criterio de `AGENTS.md` (Select es generico). Antes cerro `DEC-006B Borrador` ([DEC-006B](DEC-006B-borrador.md)) y `DEC-006A Stepper` ([DEC-006A](DEC-006A-stepper.md)). El usuario realiza los commits.
...
- Siguiente tarea sugerida: `DEC-007 Personales` (registra el paso 1 en `stepDefinitions.tsx`, usando `Dropdown` para estado civil/grado academico/agencia/oficina). Antes conviene cerrar D-03 (composicion del paso 1) y los catalogos/obligatoriedad de sus campos; si no llegan, se documentan como mock identificable. La autenticacion sigue simulada hasta disponer de contrato backend (D-08).
