...
- Ultima tarea implementada y validada: `DEC-005B Inicio y ayuda`. Cierre en [DEC-005B](DEC-005B-inicio-ayuda.md): tarjetas informativas de Inicio, modal de contacto de ayuda y estado de error recuperable (via `/inicio?demo=error`, sin inventar un servicio mock ya que el contenido es estatico). Se corrigio de paso un warning real de accesibilidad de Radix (descripcion del modal mal conectada). El usuario realiza los commits.
...
- Siguiente tarea sugerida: `DEC-006A Stepper` (wizard de doce pasos). La autenticacion sigue simulada hasta disponer de contrato backend (D-08).
