...
- Ultima tarea implementada y validada: `DEC-006A Stepper` (2026-09-21). Cierre en [DEC-006A](DEC-006A-stepper.md): doce pasos, solo actual/pendiente (sin estado "completado" porque el flujo no es lineal), tooltip con el nombre completo de cada seccion. Como shared no tenia tooltip se creo `Tooltip` en `sad-aml-shared` sobre Radix. Aun no esta montado en ninguna pantalla (lo conecta DEC-006B). El usuario realiza los commits.
...
- Siguiente tarea sugerida: `DEC-006B Borrador` (ruta `/mi-declaracion/[paso]`, conecta el stepper, Regresar/Continuar y estado del borrador). La autenticacion sigue simulada hasta disponer de contrato backend (D-08).
