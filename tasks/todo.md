...
- Ultima tarea implementada y validada: `DEC-007 Personales` (2026-09-22). Cierre en [DEC-007](DEC-007-personales.md): paso 1 completo (11 campos, D-03 resuelto con el diseno real -- solo Estado civil y Agencia son select, no cuatro como decia el inventario), registrado en `stepDefinitions.tsx`. De paso se corrigio un bug real de `Dropdown` (shared): sin un contenedor propio, dos hijos sueltos (label+trigger) descuadraban cualquier grid/flex que lo usara. Antes cerraron `DEC-002C2 Select` ([DEC-002C2](DEC-002C2-select.md)), `DEC-006B Borrador` ([DEC-006B](DEC-006B-borrador.md)) y `DEC-006A Stepper` ([DEC-006A](DEC-006A-stepper.md)). El usuario realiza los commits.
...
- Siguiente tarea sugerida: `DEC-006B2` ya esta cerrada (modal inicial); el flujo natural es `DEC-008 Familia` (paso 2) o `DEC-002E/002F` (diferidas). `DEC-008B` ya tiene la regla de obligatoriedad resuelta (D-16): la tabla puede quedar vacia, el modal de alta valida sus propios campos. La autenticacion sigue simulada hasta disponer de contrato backend (D-08).
